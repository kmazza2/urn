...
func TestUnsignedLimit(t *testing.T) {
...
func TestMaximumSeed(t *testing.T) {
	var raw_ref_str_data []byte
	var err error
	raw_ref_str_data, err = ioutil.ReadFile(`testdata/c_max_ulong_seed`)
	if err != nil {
		t.Error(`Could not open file containing test data`)
	}
	var ref_str_data []string = strings.Split(string(raw_ref_str_data), "\n")
	ref_str_data = ref_str_data[:len(ref_str_data)-1]
	var ref_data []uint64 = make([]uint64, 10)
	for i, str_num := range ref_str_data {
		var converted_num uint64
		var err error
		converted_num, err = strconv.ParseUint(str_num, 10, 64)
	...
	var data []uint64 = make([]uint64, 10)
	var rng SplitMix64 = NewSplitMix64(math.MaxUint64)
	for i := 0; i < 10; i++ {
		data[i] = rng.Next()
	}
	for i := 0; i < 10; i++ {
